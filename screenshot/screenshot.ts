  ...
  if (screenshot.focus) {
    for (const focus of screenshot.focus) {
      ...
      if (focus.type === "arrow") {
        ...
        innerHTML = `<div><span style="padding: 8px;border-radius:8px;background-color:var(--${focus.color || 'orange-50'})">${focus.content}</span></div>`
      }
      ...
      await session.page.evaluate((focus, innerHTML) => {
        ...
        if (focus.type === "arrow") {
          ...
          elementToAdd.style.alignItems = "center"
          elementToAdd.style.justifyContent = "center"
          elementToAdd.style.backgroundColor = `color-mix(in srgb, var(--${focus.color || 'orange-50'}), transparent 80%)`
